...
def main():
    html = get_index_urls(url_index_base.format(list=1809))
    doc = pq(html)
    ...
    for item in items:
        ...
        if catid:
            print('\n\n\n', catid[0], name_csv, '\n\n\n')
            i = 1
            with open(r'C:\Users\Ph\Desktop\AntAgent\maigoo\个人护理\{name}.csv'.format(name=name_csv), 'w', newline='', encoding='utf_8_sig') as csvfile:
                writer = csv.writer(csvfile)
                writer.writerow(['品牌名称','公司名称','信用评分','联系方式','企业官网','企业地址','企业邮箱','企业传真'])
                while True:
                    print('\n----------------------------------------------------------正在爬取第{page_num}页----------------------------------------------------------\n'.format(page_num=i))
                    # num: 一页爬多少个  catid: 分类编号  level: 品牌等级  0 = All
                    url_index = url_base.format(num=50, page=i, catid=catid[0], level=0)
                    # print('正在爬取： '+url_index)
                    i = i + 1
                    # time.sleep(2 + float(random.randint(1, 40))/20)
                    urls = get_list_urls(url_index)
